...
use std::{io::Write, num::NonZeroU32, ops::Sub, str, vec}; // Import the standard library // Import the hex library, used to convert bytes to hex strings and vice versa

// Import the optee_utee library, used to interact with the OP-TEE OS
use optee_utee::{
    ta_close_session, ta_create, ta_destroy, ta_invoke_command, ta_open_session, trace_println,
};
use optee_utee::{AlgorithmId, Digest, Mac, Random};
use optee_utee::{
    AttributeId, AttributeMemref, DataFlag, ObjectStorageConstants, PersistentObject,
...
use proto::{bitcoin_transaction, Command};
...
fn invoke_command(cmd_id: u32, params: &mut Parameters) -> Result<()> {
    ...
    match Command::from(cmd_id) {
        Command::CheckMasterKey => {
            ...
            check_master_key(params)
        }
        Command::GenerateMasterKey => {
            // Generate a new master key
            generate_master_key(params)
        }
        Command::MnemonicToMasterKey => {
            // Derive the master key from the mnemonic
            mnemonic_to_master_key(params)
        }
        Command::EraseMasterKey => {
            // Erase the master key
            erase_master_key(params)
        }
...
pub fn check_master_key(params: &mut Parameters) -> Result<()> {
    ...
    let param_types = (
        ParamType::ValueInput as u32,
        ParamType::None as u32,
        ParamType::None as u32,
        ParamType::None as u32,
    );
    if (
        params.0.param_type as u32,
    ...
    // Get the PIN code
    let pin = unsafe { params.0.as_value()? };
...
pub fn generate_master_key(params: &mut Parameters) -> Result<()> {
    ...
    match generate_random_mnemonic(256) {
        Err(_e) => Err(Error::new(ErrorKind::BadState))?,
        Ok(mnemonic) => {
            // Set the mnemonic in the output buffer
            let mut mnemonic_buffer = unsafe { params.1.as_memref()? };
            mnemonic_buffer
                .buffer()
            ...
            let seed: [u8; 64] = from_mnemonic_to_seed(&mnemonic, "");
            ...
            match master_seed_to_master_key(&seed) {
                ...
                Ok(ext_master_key) => {
                    ...
                    let master_key_id = vec![MASTER_KEY_ID; 8];
                    match PersistentObject::create(
                        ...
                        Ok(mut obj) => {
                            ...
                            match obj.write(&ext_master_key) {
                                Ok(_n) => {
                                    trace_println!(
                                        "Extended Master Key base10 {:?}",
                                        U256::from_big_endian(&ext_master_key[0..32])
                                    );
                                    trace_println!(
                                        "Extended Master Key base16 {:?}",
                                        hex::encode(&ext_master_key[0..32])
                                    );
                                    Ok(())
                                }
                                Err(e) => Err(e),
...
pub fn mnemonic_to_master_key(params: &mut Parameters) -> Result<()> {
    ...
    // Generate the seed based on the mnemonic
    let seed: [u8; 64] = from_mnemonic_to_seed(
        str::from_utf8(mnemonic).map_err(|_| Error::new(ErrorKind::BadParameters))?,
        "",
    );
...
pub fn erase_master_key(params: &mut Parameters) -> Result<()> {
    ...
    match PersistentObject::open(
        ...
        Ok(obj) => {
            PersistentObject::close_and_delete(obj)?;
            Ok(())
        }
                        ...
                        Ok(bitcoin_address) => {
                            ...
                            match base_58_check_encode(bitcoin_address, true) {
                                ...
                                Ok(bitcoin_address_base58) => {
                                    ...
                                    let mut output_buffer = unsafe { params.1.as_memref()? };
                                    output_buffer
                                        .buffer()
                                        .write_all(&bitcoin_address_base58)
                                        .map_err(|_| Error::new(ErrorKind::BadState))?;
                                    Ok(())
                                }
                            }
...
pub fn sign_transaction(params: &mut Parameters) -> Result<()> {
    ...
    let mut transaction_data = unsafe { params.1.as_memref()? };
    let transaction_data_buffer = transaction_data.buffer();
    trace_println!("Transaction data: {:?}", transaction_data_buffer);
    let mut transaction: Transaction = Transaction::deserialize(transaction_data_buffer);
    trace_println!("Transaction data: {:?}", transaction);

    // Append the SIGHASH_ALL flag to the transaction data
    let tx_data = [transaction_data_buffer.to_vec(), [1u8].to_vec()].concat();
    ...
    let script_sig = [
        bitcoin_transaction::var_int(der_signature.len() + 1),
        der_signature.to_vec(),
        [1u8].to_vec(),
        [33u8].to_vec(),
        public_key.to_vec(),
    ]
    ...
    // Set the output buffer
    let mut out_buffer = unsafe { params.2.as_memref()? };
    out_buffer
        .buffer()
        .write_all(&serialized_transaction)
        .map_err(|_| Error::new(ErrorKind::BadState))?;
...
fn from_mnemonic_to_seed(mnemonic: &str, passphrase: &str) -> [u8; 64] {
    // Define the salt
...
fn check_if_master_key_exist() -> Result<()> {
    ...
    match PersistentObject::open(
        ...
        Ok(mut _obj) => Ok(()),
        // If the master key does not exist, return an error
        Err(_e) => Err(Error::new(ErrorKind::ItemNotFound)),
    }
}
...
fn read_master_key() -> Result<[u8; 64]> {
    ...
    match PersistentObject::open(
        ...
        Err(_e) => {
            ...
            Err(Error::new(ErrorKind::ItemNotFound))
        }
        // If the master key exists, contiue
        Ok(obj) => match obj.read(&mut master_key_ext) {
            Err(_e) => Err(Error::new(ErrorKind::AccessDenied)),
            Ok(_size) => Ok(master_key_ext),
        },
    }
}
...
fn generate_private_child_key(level: u32, index: u32) -> Result<[u8; 32]> {
        ...
        Ok(mac) => {
            ...
            match TransientObject::allocate(TransientObjectType::HmacSha512, 512) {
                ...
                Ok(mut key) => {
                    ...
                    let attr = AttributeMemref::from_ref(
                        AttributeId::SecretValue,
                        &master_key_ext[32..64],
                    );
                    // Populate the key attributes
                    key.populate(&[attr.into()])?;
                    // Set the key in the HMAC operation
    ...
    trace_println!(
        "Parent private key base16: {:?}",
        hex::encode(&master_key_ext[0..32])
    );
    trace_println!(
        "Parent private Chain Code base16: {:?}",
        hex::encode(&master_key_ext[32..64])
    );
    ...
    if i_l_u512.cmp(&curve_order) == core::cmp::Ordering::Greater
        || i_l_u512.cmp(&curve_order) == core::cmp::Ordering::Equal
    {
    ...
    let child_private_key_u256 = if res >= curve_order {
        U256::from(res.sub(curve_order))
    } else {
        U256::from(res)
    };
...
fn generate_public_key_compressed(private_key: [u8; 32]) -> Result<[u8; 33]> {
    ...
    let cprvk =
        SecretKey::from_slice(&private_key).map_err(|_| Error::new(ErrorKind::BadParameters))?;
    ...
    let aux =
        hex::decode(public_key_str.clone()).map_err(|_| Error::new(ErrorKind::BadParameters))?;
    for (i, byte) in aux.into_iter().enumerate() {
        public_key[i] = byte;
    }
...
fn generate_bitcoin_address(public_key: [u8; 33]) -> Result<[u8; 20]> {
    ...
    match Digest::allocate(AlgorithmId::Sha256) {
        Err(_e) => Err(Error::new(ErrorKind::BadState))?,
        Ok(sha256) => match sha256.do_final(&public_key, &mut sha256_hash) {
            Err(_e) => Err(Error::new(ErrorKind::BadState))?,
            Ok(n) => trace_println!(
                "First stage of the SHA256 operation completed with: {} bytes",
                n
            ),
        },
    }
...
fn base_58_check_encode(ripemd160_hash: [u8; 20], addr_type: bool) -> Result<Vec<u8>> {
    ...
    match Digest::allocate(AlgorithmId::Sha256) {
        ...
        Ok(sha256) => {
            ...
            match sha256.do_final(&ripemd160_in, &mut sha256_hash2) {
                Err(_e) => Err(Error::new(ErrorKind::BadState))?,
                Ok(n) => trace_println!(
                    "Third stage of the SHA256 operation completed with: {} bytes",
                    n
                ),
            }
        }
    ...
    match Digest::allocate(AlgorithmId::Sha256) {
        Err(_e) => Err(Error::new(ErrorKind::BadState))?,
        Ok(sha256) => match sha256.do_final(&sha256_hash2, &mut sha256_hash3) {
            Err(_e) => Err(Error::new(ErrorKind::BadState))?,
            Ok(n) => trace_println!(
                "Four stage of the SHA256 operation completed with: {} bytes",
                n
            ),
        },
    }
...
fn sign(private_key: [u8; 32], public_key: [u8; 33], message: &[u8]) -> Result<Vec<u8>> {
    ...
    match Digest::allocate(AlgorithmId::Sha256) {
        ...
        Ok(sha256) => match sha256.do_final(message, &mut sha256_hash) {
            Err(_e) => {
    ...
    let msg =
        Message::from_slice(&sha256_hash).map_err(|_| Error::new(ErrorKind::BadParameters))?;
    ...
    let pub_key =
        PublicKey::from_slice(&public_key).map_err(|_| Error::new(ErrorKind::BadParameters))?;
